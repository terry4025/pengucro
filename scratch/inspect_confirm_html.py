...
def check_and_book_and_inspect(session, csrf_token):
    ...
    # Step 3: GET /reservation/confirm
    res_confirm = session.get(f'{BASE_URL}/reservation/confirm')
    res_confirm.encoding = 'utf-8'
    
    # Save the HTML to scratch/confirm.html
    with open('scratch/confirm.html', 'w', encoding='utf-8') as f:
        f.write(res_confirm.text)
    print("Saved HTML to scratch/confirm.html")
    
    # Search for keywords in raw text
    raw_text = res_confirm.text
    print("\nSearch results:")
...
check_and_book_and_inspect(session, csrf)
