...
async def main():
    ...
    for i in range(10):
        ...
        # Try different thresholds and binarization styles
        gray = img.convert("L")
        # Scale up
        scaled = gray.resize((img.size[0] * 3, img.size[1] * 3), Image.Resampling.LANCZOS)
        
        # Try binarize at 120
        binarized = scaled.point(lambda p: 255 if p > 120 else 0)
        proc_path = f"scratch/test_cap_proc_{i}.png"
        binarized.save(proc_path)
        
        text = await ocr_image(proc_path)
        clean = "".join(c for c in text if c.isalnum())
        print(f"Sample {i}: Raw='{text}', Clean='{clean}'")
