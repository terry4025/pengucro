...
PATCH_NOTES: tuple[PatchNote, ...] = (
    PatchNote(
        version="6.85",
        changes=(
            "키이스케이프 연결 지연 보정이 실제 예약 전송에 반영되지 않던 문제 수정",
            "키이스케이프 최대 10분 취소표 대기 선택 기능 추가",
            "키이스케이프 예상 시간표와 실제 슬롯 검증 로그 구분",
            "키이스케이프 예약 전송 후 화면 응답 오류 시 중복 시도 방지",
            "제로월드·신비월드 요청 제한 시 서버가 안내한 대기시간 준수",
            "제로월드·신비월드 예약 준비 중 중지 반영과 중복 대기 개선",
            "제로월드·신비월드 긴 대기 후 오래된 인증값으로 제출하던 문제 방지",
            "CGV 장시간 감시 중 로그인 만료 확인과 세션 복구 개선",
            "CGV 인증 확인 응답 지연과 오래된 응답 처리 개선",
            "사용이 종료된 제로월드 구형 엔진 제거",
        ),
    ),
    PatchNote(
        version="6.84",
        changes=(
            "네이버 오픈 직전 시각 보정으로 제출이 늦어지던 문제 개선",
            "네이버 오픈 직전 불필요한 조회와 준비 대기 단축",
            "네이버 여러 실행 간 중복 제출과 예약 탭 간섭 방지",
            "네이버 불명확한 제출 응답의 예약내역 대조와 결제 화면 복구 강화",
            "네이버 접수·결제 대기와 완료 상태 구분 및 성공 오표시 방지",
            "네이버 응답 지연 시 예약·결제 버튼 중복 클릭 방지",
        ),
    ),
    PatchNote(
        version="6.83",
    ...
    PatchNote(
        ...
        changes=(
            ...
            "단편선 주문 거절 사유 표시와 불명확한 응답의 중복 제출 방지 강화",
            "단편선 오래된 예약 가능 상태 재확인 및 접근 제한 시 자동 중단",
            "단편선 실제 시간표 확인 공백과 요청 대기 진단 개선",
    ...
    PatchNote(
        version="6.81",
        changes=(
            "제로월드 숫자 자동입력 인증 인식 및 제출 전 검증 개선",
            "제로월드 실제 예약내역 확인으로 접수 성공 오표시와 중복 제출 방지 강화",
            "제로월드 미오픈 날짜에 최근 같은 요일의 예상 시간표 표시",
    ...
    PatchNote(
        ...
        changes=(
            ...
            "단편선 슬롯 발견 후 예약 진행 우선 처리와 중지 동작 개선",
            "단편선 요청 대기 시간과 서버 응답 시간 진단 분리",
    ...
    PatchNote(
        version="6.77",
        changes=(
            "단편선 시작 시 브라우저를 백그라운드에서 준비하고 접수 완료 화면만 표시",
            "단편선 동시 감시 슬라이더 최대 32개 지원",
            "여러 프로그램에서 단편선 감시 요청량과 서버 지연 대기를 함께 조절",
            "단편선 사전 준비 브라우저의 4개 실행 제한 해소",
        ),
    ),
    PatchNote(
        version="6.76",
