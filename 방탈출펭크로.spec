# -*- mode: python ; coding: utf-8 -*-
a = Analysis(
    ['app.py'],
    pathex=[],
    binaries=[],
    datas=[('icon.ico', '.'), ('app_icon.png', '.')],
    hiddenimports=[
        # Engines are selected by name at runtime in engines/registry.py, so
        # static analysis cannot see them.
        'engines.zeroworld_shin_engine',
        'engines.doomescape_engine',
        'engines.zeroworld_gu_engine',
        'engines.jigubyeol_engine',
        'engines.async_hot_path',
        'engines.keyescape_engine',
        'engines.keyescape_engine_single_page',
        'engines.keyescape_engine_observed',
        'engines.keyescape_engine_runtime',
        'engines.dpsnnn_engine',
        'engines.naver_engine',
        'engines.cgv_engine',
        'engines.cgv_engine_hardened',
        'engines.cgv_engine_guarded',
        'engines.cgv_engine_runtime',
        'engines.cgv_engine_watchdog',
        'engines.cgv_engine_pairwise',
        'engines.cgv_engine_pairwise_observer',
        'engines.cgv_engine_visitor_runtime',
        'engines.cgv_engine_visitor_dom_runtime',
        'engines.cgv_engine_priority_ladder',
        'engines.cgv_engine_priority_ladder_runtime',
        'engines.cgv_engine_movie_identity_runtime',
        'engines.cgv_engine_funnel_runtime',
        'engines.cgv_engine_preopen_sentinel_runtime',
        'engines.cgv_engine_preopen_live_runtime',
        'engines.cgv_movie_identity',
        'engines.cgv_preopen_matching',
        'engines.cgv_yongsan_preopen_presets',
        'engines.cgv_client',
        'engines.cgv_browser_client',
        'engines.cgv_browser_client_runtime',
        'engines.cgv_chrome_session',
        'engines.npay_keypad_recognizer',
        'engines.yescaptcha_client',
        'playwright.async_api',
        'playwright.sync_api',
        'win32crypt',
        # Modules added in the 5.32/5.4 reworks. They are imported normally, so
        # static analysis cannot see them.  Keep the final selector layer too.
        'ui.loading_overlay',
        'ui.repaint',
        'ui.scrollable',
        'ui.cgv_booking_dialog',
        'ui.cgv_booking_dialog_runtime',
        'ui.cgv_booking_dialog_controls',
        'ui.cgv_booking_dialog_preopen_auto',
        'ui.cgv_booking_dialog_movie_runtime',
        'ui.cgv_booking_dialog_yongsan_preopen',
        'ui.reservation_form',
        'ui.reservation_form_runtime',
        'ui.update_dialog',
        'pengucro.logging_setup',
        # The updater is partly loaded on demand. Keep its controller, helper,
        # manifest verifier, and cryptographic Ed25519 runtime in one-file
        # builds even when static import analysis cannot see every branch.
        'pengucro.updater',
        'pengucro.update_manifest',
        'pengucro.update_helper',
        'pengucro.update_bootstrap',
        'pengucro.update_controller',
        'cryptography',
        'cryptography.hazmat.bindings._rust',
        'cryptography.hazmat.primitives.serialization',
        'cryptography.hazmat.primitives.asymmetric.ed25519',
        'engines.browser_session',
        'engines.server_clock',
        'engines.naver_api',
        'engines.naver_forms',
        'engines.catchtable_engine',
        'engines.catchtable_client',
        'engines.catchtable_crypto',
        'engines.catchtable_browser',
        'engines.catchtable_models',
    ],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=['pydantic', 'pydantic_core'],
    noarchive=False,
    optimize=0,
)
pyz = PYZ(a.pure)
exe = EXE(
    pyz,
    a.scripts,
    a.binaries,
    a.datas,
    [],
    name='방탈출펭크로6.54_yescaptcha',
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=True,
    upx_exclude=[],
    runtime_tmpdir=None,
    console=False,
    disable_windowed_traceback=False,
    argv_emulation=False,
    target_arch=None,
    codesign_identity=None,
    entitlements_file=None,
    icon=['icon.ico'],
)
