...
def test_v653_release_contract_is_complete():
    assert __version__ == "6.53"
    assert __release_sequence__ == 6530001
    note = notes_for("6.53")
    assert note is not None
    assert any("스케줄링" in change for change in note.changes)
