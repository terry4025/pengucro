...
def test_v631_release_contract_is_complete():
    assert __version__ == "6.31"
    assert __release_sequence__ == 6310001
    note = notes_for("6.31")
    assert note is not None
    assert any("미오픈" in change or "지연" in change for change in note.changes)
