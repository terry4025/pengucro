...
def test_v633_release_contract_is_complete():
    assert __version__ == "6.33"
    assert __release_sequence__ == 6330001
    note = notes_for("6.33")
    assert note is not None
    assert any("좌석도" in change or "우선순위" in change for change in note.changes)
