...
def test_v652_release_contract_is_complete():
    assert __version__ == "6.52"
    assert __release_sequence__ == 6520001
    note = notes_for("6.52")
    assert note is not None
    assert any("사전 준비" in change for change in note.changes)
