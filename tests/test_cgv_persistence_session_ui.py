...
def test_v632_release_contract_is_complete():
    assert __version__ == "6.32"
    assert __release_sequence__ == 6320001
    note = notes_for("6.32")
    assert note is not None
    assert any("장시간" in change or "감시" in change for change in note.changes)
