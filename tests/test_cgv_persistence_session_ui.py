...
def test_v635_release_contract_is_complete():
    assert __version__ == "6.35"
    assert __release_sequence__ == 6350001
    note = notes_for("6.35")
    assert note is not None
    assert any("좌석" in change or "파트너" in change for change in note.changes)
