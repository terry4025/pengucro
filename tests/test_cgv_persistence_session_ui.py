...
def test_v630_release_contract_is_complete():
    assert __version__ == "6.30"
    assert __release_sequence__ == 6300001
    note = notes_for("6.30")
    assert note is not None
    assert any("우선순위" in change for change in note.changes)
