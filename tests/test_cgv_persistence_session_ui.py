...
def test_v641_release_contract_is_complete():
    assert __version__ == "6.41"
    assert __release_sequence__ == 6410001
    note = notes_for("6.41")
    assert note is not None
    assert any("키패드" in change or "네이버페이" in change for change in note.changes)
