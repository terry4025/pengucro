...
def test_v651_release_contract_is_complete():
    assert __version__ == "6.51"
    assert __release_sequence__ == 6510001
    note = notes_for("6.51")
    assert note is not None
    assert any("일괄 저장" in change for change in note.changes)
