...
def test_v634_release_contract_is_complete():
    assert __version__ == "6.34"
    assert __release_sequence__ == 6340001
    note = notes_for("6.34")
    assert note is not None
    assert any("결제" in change or "시그니처" in change for change in note.changes)
